...
import 'package:food_topia/domain/usecases/get_meals_data_test.mocks.dart';


@GenerateMocks([MealsDataRepository])
void main() {
  MockMealsDataRepository mealsDataRepositoryTest = MockMealsDataRepository();
  GetMealsDataByName usecase = GetMealsDataByName(mealsDataRepositoryTest);

  final name = "Bakewell tart";
  ...
  test('Get Meals Data By Name from Repositories', () async {
    when(mealsDataRepositoryTest.getMealsDataByName(any))
        .thenAnswer((_) async => Right(tMealsData));
    final result = await usecase.execute(name: name);
    expect(result, Right(tMealsData));
    verify(mealsDataRepositoryTest.getMealsDataByName(name));
    verifyNoMoreInteractions(mealsDataRepositoryTest);
  });
}
// class MealsDataRepositoryTest extends Mock implements MealsDataRepository {
//
//
// }
